...
void makeplotsenergy_unified(std::string filename, int scinttype) {
  ...
  for (int i = 0; i < ScintLenvsKE->GetXaxis()->GetNbins(); ++i) {
    ...
    TFitResult *res = result.Get();
    GaussEst->SetBinContent(i+1, res->GetParams()[1]);
    GaussEst->SetBinError(i+1, res->GetParams()[2]);
