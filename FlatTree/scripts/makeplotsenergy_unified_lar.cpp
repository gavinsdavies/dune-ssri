...
void makeplotsenergy_unified_lar(std::string filename, int scinttype) {
  ...
  for (int i = 0; i < nEntries; ++i) {
    ...
    // Fiducial cut in TMS in z for events stopping in TMS
    if (muonReco == 2 && 
          (muonBirth[2] > 733. || muonDeath[2] > 1375. || 
          muonDeath[1] > 25.+50. || muonDeath[1] < -260.+50. ||
          abs(muonDeath[0]) > 300. ||
          muonExitKE <= 0 ||
          abs(muonDeath[0]) > 165. || abs(muonDeath[0]) < 10. )) continue;

    // Recalculate the scintillation length only for TMS contained
    if (muonReco == 2) {
      muScintLen -= 24.35;
  ...
  canvname.ReplaceAll(".root", "muonKEest_xtratrackfix_uni_larlen_seg_LArTMScont");
  ...
  for (int i = 0; i < nEntries; ++i) {
    ...
    // Skip events that don't stop in TMS or LAr
    if (muonReco == 0) continue;
